...
average membrane potentials) of local populations \cite{tewarie2021validity}. A classic example is the Wilson–Cowan model,
which couples excitatory (E) and inhibitory (I) populations in each region using sigmoidal activation
functions \cite{wilson1972excitatory,wilson1973dynamics}. This E/I architecture embodies the excitation–inhibition balance thought critical for
stable cortical function \cite{liang2025excitation}. Balanced E/I drive maintains neurons in an appropriate operating range, so that
shifts in excitation or inhibition produce changes in excitability \cite{liang2025excitation}. In whole-brain modeling, each brain
...
representing the “wiring diagram” among regions \cite{sporns2005human,hagmann2005diffusion}. In practice, large-scale connectomes are derived
from diffusion-weighted MRI (diffusion tensor imaging, or DTI) and tractography, which noninvasively trace
white-matter fiber pathways in the living human brain \cite{mukherjee2023diffusion}. The output is a structural connectivity
matrix, whose entries encode the strength (e.g. fiber count or probability) of connections between region
pairs (and often associated fiber lengths for conduction delays) \cite{jimenezmarin2024open}. This structural connectivity
constrains how activity flows between regions, shaping network dynamics \cite{honey2007network,deco2009key}. \\
...
reflects local changes in blood oxygenation caused by neural activity and hemodynamic responses \cite{kim2012biophysical}. In
...
GPU support \cite{kidger2021equinox}; Diffrax for differential-equation solvers \cite{kidger2021neural}) to speed up simulations, while preserving the
...
diffusion in each voxel indicates underlying fiber orientation \cite{mukherjee2023diffusion}. Sophisticated reconstruction algorithms then
...
brain’s structural connectivity \cite{mukherjee2023diffusion}.\\
...
(i, j), tractography yields a count or probability of streamlines (axon-like trajectories) connecting i to j \cite{jimenezmarin2024open}. These
values form the elements of a weighted adjacency matrix. In practice, the SC matrix entry $C_{ij}$ may
represent the fraction or number of fibers from region j that reach region i \cite{honey2007network}. Because tractography can
...
Structural connectivity matrices are sometimes called the “highways” of the brain \cite{sporns2005human}: they constrain how activity in one region can influence distant regions. Many modeling studies have shown that the SC matrix strongly shapes emergent dynamics, including the observed patterns of functional connectivity (statistical correlations) in 
fMRI and EEG \cite{honey2007network,deco2009key,breakspear2017dynamic}. In our Neurolib models, the connectivity matrix (fiber counts or weights) and length matrix (fiber lengths) are loaded as key model parameters. For example, in the BaseModel we store $fiber\_count\_matrix$ and $fiber\_length\_matrix$ and construct $connectivity\_matrix$ by zeroing self-connections (diagonals), as in other network models.
...
connecting the two regions \cite{jimenezmarin2024open}. The absolute scale of the weights may be normalized 
...
independent dynamics \cite{honey2007network}.
...
synchronization \cite{deco2009key}.
...
mechanism \cite{kidger2021neural}, which handles possibly many distinct delay lengths. (The code precomputes unique delays and
...
(e.g. hundreds of ROIs) \cite{jimenezmarin2024open}. Our BaseModel supports an arbitrary square connectivity matrix. We also perform
...
JIT compilation and GPU execution when performing the simulation \cite{kidger2021equinox}.
...
PyTrees of JAX arrays \cite{kidger2021equinox}. Key attributes of BaseModel include:\\
...
index in the unique-delay list. This allows diffrax’s Delay handling \cite{kidger2021neural} to efficiently reuse repeated delays.\\
...
constructs a time array ts = arange(0, duration, dt) and calls diffrax.diffeqsolve \cite{kidger2021neural}. Key
details:\\
...
be JIT-compiled and run on a GPU if available \cite{kidger2021equinox}. Using JAX arrays for state and parameters means the model is
...
“static” parts and then applies jax.grad to the sum of outputs \cite{kidger2021equinox}. This allows parameter optimization or
...
diffrax’s advanced solvers and delay management \cite{kidger2021neural} simplifies the code: the complex loop over time with
...
region, inheriting from \texttt{BaseModel} \cite{wilson1972excitatory,wilson1973dynamics}. It defines the excitatory--inhibitory (E--I)
...
are part of the model’s PyTree and can be differentiated \cite{kidger2021equinox}.
...
as typical in Wilson--Cowan models \cite{wilson1972excitatory,wilson1973dynamics}.
...
\end{itemize}
The system is then integrated by Diffrax \cite{kidger2021neural}.
...
differentiation, and parallel execution on accelerator hardware \cite{kidger2021equinox}.
...
example, Fellous et al.\ \cite{fellous2003synaptic} simulated in vivo--like synaptic bombardment using OU processes for
...
OU state, which generates the Wiener increments over time \cite{kidger2021neural}.
\end{itemize}
...
which can better mimic synaptic filtering \cite{fellous2003synaptic}. Also, because diffrax handles SDEs in Stratonovich form, the
...
standard solver (Neurolib uses a fixed-step Euler method).
...
The test results confirmed that both models are producing the same dynamics for deterministic parameters, which shows the neurolib 2 calculations are correct and precise.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{wilson_cowan_dynamics.png}
    \caption{Simulation of the Wilson--Cowan model across 80 brain regions using Neurolib~2. 
    The figure illustrates the spatiotemporal dynamics of excitatory and inhibitory populations, 
    demonstrating inter-regional interactions mediated by structural connectivity and conduction delays. 
    Stochastic fluctuations are introduced via Ornstein--Uhlenbeck noise, highlighting variability 
    around deterministic trajectories.}
    \label{fig:wilson_cowan_dynamics}
\end{figure}
...
classes. By harnessing JAX, Equinox, and Diffrax \cite{kidger2021equinox,kidger2021neural}, we have reimplemented neural-mass models with GPU-
accelerated solvers and built-in differentiation. Our results show that even at this early stage, the new code
reproduces established Wilson--Cowan dynamics with high fidelity.

Key advantages of this approach include:
\begin{itemize}
\item \textbf{Speed and parallelism:} JAX allows automatic JIT compilation and parallel execution on GPUs or
TPUs \cite{kidger2021equinox}. The vectorized implementation can handle many regions simultaneously without Python loops.

\item \textbf{Reliability and maintainability:} Using diffrax’s integrators \cite{kidger2021neural} and JAX’s functional style reduces
...
which was not straightforward in Neurolib~1 \cite{kidger2021equinox}.

\item \textbf{Stochastic modeling:} Incorporating OU noise with diffrax allows easy experimentation with noisy
dynamics, supporting more biologically realistic simulations \cite{fellous2003synaptic}.
\end{itemize}
...
\bibitem{tewarie2021validity}
Tewarie P, et al. (2021). On the Validity of Neural Mass Models. \emph{Frontiers in Computational Neuroscience}, 14:581040.

\bibitem{wilson1972excitatory}
Wilson HR, Cowan JD (1972). Excitatory and inhibitory interactions in localized populations of model neurons. \emph{Biophysical Journal}, 12(1):1-24.

\bibitem{wilson1973dynamics}
Wilson HR, Cowan JD (1973). A mathematical theory of the functional dynamics of cortical and thalamic nervous tissue. \emph{Kybernetik}, 13(2):55-80.

\bibitem{liang2025excitation}
Liang J, et al. (2025). Excitation-Inhibition Balance, Neural Criticality, and Activities in Neuronal Circuits. \emph{The Neuroscientist}, 31(2):145-162.

\bibitem{sporns2005human}
Sporns O, Tononi G, Kötter R (2005). The human connectome: A structural description of the human brain. \emph{PLoS Computational Biology}, 1(4):e42.

\bibitem{hagmann2005diffusion}
Hagmann P (2005). From diffusion MRI to brain connectomics. PhD Thesis, École Polytechnique Fédérale de Lausanne.

\bibitem{mukherjee2023diffusion}
Mukherjee P, et al. (2023). Diffusion tensor MR imaging and fiber tractography: theoretic underpinnings. \emph{American Journal of Neuroradiology}, 44(3):267-278.

\bibitem{jimenezmarin2024open}
Jimenez-Marin A, et al. (2024). Open datasets and code for multi-scale relations on structure, function and neuro-genetics in the human brain. \emph{Scientific Data}, 11(1):82.

\bibitem{honey2007network}
Honey CJ, et al. (2007). Network structure of cerebral cortex shapes functional connectivity on multiple time scales. \emph{Proceedings of the National Academy of Sciences}, 104(24):10240-10245.

\bibitem{deco2009key}
Deco G, Jirsa V, McIntosh AR, Sporns O, Kötter R (2009). Key role of coupling, delay, and noise in resting brain fluctuations. \emph{Proceedings of the National Academy of Sciences}, 106(25):10302-10307.

\bibitem{breakspear2017dynamic}
Breakspear M (2017). Dynamic models of large-scale brain activity. \emph{Nature Neuroscience}, 20(3):340-352.

\bibitem{kim2012biophysical}
Kim SG, Ogawa S (2012). Biophysical and physiological origins of blood oxygenation level-dependent fMRI signals. \emph{Journal of Cerebral Blood Flow \& Metabolism}, 32(7):1188-1209.

\bibitem{kidger2021equinox}
Kidger P, Garcia C (2021). Equinox: neural networks in JAX via callable PyTrees and filtered transformations. \emph{Differentiable Programming workshop at NeurIPS 2021}.

\bibitem{kidger2021neural}
Kidger P (2021). On Neural Differential Equations. PhD Thesis, University of Oxford.

\bibitem{fellous2003synaptic}
Fellous JM, Rudolph M, Destexhe A, Sejnowski TJ (2003). Synaptic background noise controls the input/output characteristics of single cells in an in vitro model of in vivo activity. \emph{Neuroscience}, 122(3):811-829.
